...
struct meaning_word { // record information of a sememe
  char *word;
};
...
real *meaning_syn; // sememe embedding, similar to word embedding

struct sense { // record information of several senses of a word
  int num; // number of senses 
  real *mult_sense_value; // sense embeddings
  int *meaning_word_cnt; // numbers of sememes for each sense
...
void TrainModel() {
  ...
  if (classes == 0) {
    // Save the word vectors. If there exist sense embeddings and sememe embeddings, save them.
